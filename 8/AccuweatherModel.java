    ...
    public void getWeather(String selectedCity, Period period) throws IOException, SQLException {
        switch (period) {
            case NOW:
                ...
                System.out.print("Температура воздуха на  ");
                while (iterator.hasNext()) {
                    JsonNode day = iterator.next();
                    System.out.print(day.path("Date").asText().substring(0,10) + "   ");
                    System.out.println("Минимум " + day.path("Temperature").path("Minimum").path("Value").asText() + " " +
                            ...
                            day.path("Temperature").path("Minimum").path("Unit").asText());
                    System.out.println(" ");
                    Weather weather = new Weather(selectedCity, day.path("Date").asText().substring(0,10),
                            day.path("Temperature").path("Minimum").path("Value").asDouble(),
                            day.path("Temperature").path("Maximum").path("Value").asDouble() );
            ...
            case FIVE_DAYS:
                ...
                while (iterator5.hasNext()) {
                    ...
                    System.out.println("Дата " + day.path("Date").asText().substring(0,10) +
                            "    Минимум " + day.path("Temperature").path("Minimum").path("Value").asText() + " " +
                            day.path("Temperature").path("Minimum").path("Unit").asText() +
                            "    Максимум " + day.path("Temperature").path("Maximum").path("Value").asText() + " " +
                            day.path("Temperature").path("Minimum").path("Unit").asText());

                    weather.add(new Weather(selectedCity, day.path("Date").asText().substring(0,10),
                            day.path("Temperature").path("Minimum").path("Value").asDouble(),
                            day.path("Temperature").path("Maximum").path("Value").asDouble() ));
                }
                System.out.println(" ");
